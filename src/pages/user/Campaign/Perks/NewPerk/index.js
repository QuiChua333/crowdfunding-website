...
function NewPerk() {
    ...
    const handleCheckShipping = () => {
        ...
        setPerkState(prev => ({
            ...prev, isShipping: true,
            listShippingFee: [...prev.listShippingFee].length === 0 ? [{ location: 'Tất cả các tỉnh thành', fee: '' }] : prev.listShippingFee
        }))
    }
        ...
        if (lengthListShippingFee === 0 || lengthListShippingFee === 1) {
            ...
            setPerkState(prev => ({
                ...
                listShippingFee: [...prev.listShippingFee].map(item => {
                    if (item.location === 'Các tỉnh thành còn lại') return {
                        ...item, location: 'Tất cả các tỉnh thành'
                    }
                    else return item
                })
            }))
            ...
            setPerkState(prev => ({
                ...prev,
                listShippingFee: [...prev.listShippingFee].map(item => {
                    if (item.location === 'Tất cả các tỉnh thành') return {
                        ...item, location: 'Các tỉnh thành còn lại'
                    }
                    else return item
                })
            }))
                        ...
                        <div className={cx('body')}>
                            <div className={cx('entreSection')}>
                                ...
                                <div className={cx('entreField-subHeader')}>
                                    Đặc quyền là những ưu đãi được cung cấp cho những người ủng hộ để đổi lấy sự hỗ trợ của họ. Hãy chắc chắn rằng đặc quyền của bạn không bị cấm.
                                </div>

                                <div className={cx('entreField')}>
                                    <label className={cx('entreField-label')}>Chế độ hiển thị</label>
                                    <div className={cx('entreField-subLabel')}>
                                        Bạn có thể thay đổi mức độ hiển thị của các đặc quyền của mình bất kỳ lúc nào. Thay đổi chế độ hiển thị thành ẩn nếu bạn đang làm việc với một đặc quyền chưa sẵn sàng hoặc nếu bạn không muốn người ủng hộ yêu cầu đặc quyền đó nữa.
                                    </div>
                                    <div style={{ marginTop: '16px' }}>
                                        <label className={cx('inputRadioGroup-radio')}>
                                            <input type="radio" value={'VSBL'} name="perkVisibility" defaultChecked onChange={handleCheckVisible} checked={perkState.isVisible} />
                                        ...
                                        <label className={cx('inputRadioGroup-radio')}>
                                            <input type="radio" value={'INVS'} name="perkVisibility" onChange={handleCheckHidden} checked={!perkState.isVisible} />
                                            <span className={cx('inputRadioGroup-radio-button')}></span>
                                            <span className={cx('inputRadioGroup-radio-label')}>
                                ...
                                <div className={cx('entreField')}>
                                    <label className={cx('entreField-label')}>Trị giá<span className={cx('entreField-required')}>*</span></label>
                                    <div className={cx('entreField-subLabel')}>
                                        Đặt số tiền bạn muốn thu từ những người ủng hộ yêu cầu đặc quyền này. Số tiền này sẽ thể hiện số tiền bạn muốn nhận cho tất cả các vật phẩm có trong đặc quyền này.
                                    </div>
                                    <div className={cx('inputCurrencyField')}>
                                        <span className={cx('inputCurrencyField-symbol')}>$</span>
                                        <input type="text" maxLength="50" className={cx('itext-field', 'inputCurrencyField-input')} name="price" value={perkState.price} onChange={handleChangeInputText} />
                                ...
                                <div className={cx('entreField')}>
                                    ...
                                    <div className={cx('entreField-subLabel')}>
                                        Tiêu đề cho đặc quyền của bạn là tiêu đề sẽ xuất hiện trên trang chiến dịch của bạn và trên khắp GIVE - FUN. Tạo tiêu đề mô tả đúng nhất nội dung của đặc quyền này.
                                    </div>
                                    <input type="text" className={cx('itext-field')} name="title" value={perkState.title} onChange={handleChangeInputText} />
                                    <div className={cx('entreField-validationLabel')}>50</div>
                                </div>

                                <div className={cx('entreField')}>
                                    <label className={cx('entreField-label')}>Các vật phẩm <span className={cx('entreField-required')}>*</span></label>
                                    <div className={cx('entreField-subLabel')}>
                                        Thêm các vật phẩm có trong đặc quyền này. Vật phẩm có thể có giá trị vật chất, tinh thần, sự trải nghiệm hoặc thậm chí chỉ là một lời cảm ơn. Chỉ định số lượng mặt hàng và thêm các mặt hàng bổ sung để tạo gói đặc quyền.
                                    </div>
                                    ...
                                    <div onClick={handleClickAddItem} style={{ padding: '16px 0', cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}>
                                        <span style={{ width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#eee5f2', color: '#7a69b3', borderRadius: '50%', marginRight: '12px' }}>
                                ...
                                <div className={cx('entreField')}>
                                    <label className={cx('entreField-label')}>Mô tả <span className={cx('entreField-required')}>*</span></label>
                                    <div className={cx('entreField-subLabel')}>
                                        Mô tả chi tiết về đặc quyền này. Hãy sáng tạo, đây là cơ hội để bạn thu hút những người ủng hộ bằng cách cho họ biết về những gì họ sẽ nhận được sau khi yêu cầu đặc quyền này.
                                    </div>
                                    <textarea className={cx('itext-field')} style={{ minHeight: '275px' }} name="description" value={perkState.description} onChange={handleChangeInputText}></textarea>
                                ...
                                <div className={cx('entreField')}>
                                    ...
                                    <div className={cx('entreField-subLabel')}>
                                        Vui lòng không sử dụng hình ảnh có chứa văn bản như giá cả và mức giảm giá hoặc màu sắc của thương hiệu GIVE - FUN. Kích thước được đề xuất: 660x440 pixel. Hỗ trợ PNG hoặc JPG.
                                    </div>
                                    <div>
                                ...
                                <div className={cx('entreField')}>
                                    ...
                                    <div className={cx('entreField-subLabel')}>
                                        Bạn có thể giới hạn số lượng có sẵn cho người ủng hộ dựa trên khối lượng sản xuất. Để trống trường này có nghĩa là không có giới hạn số lượng.
                                    </div>
                                    <input type="text" className={cx('itext-field')} style={{ width: '200px' }} name="quantity" value={perkState.quantity} onChange={handleChangeInputText} />
                                </div>
                            ...
                            <div className={cx('entreSection')}>
                                ...
                                <div style={{ marginTop: '32px' }}>
                                    <label onClick={handleCheckNoShipping} className={cx('inputRadioGroup-radio')} >
                                        <input type="radio" value={'VSBL'} name="shippingAddressRequired" defaultChecked checked={!perkState.isShipping} />
                                        <span className={cx('inputRadioGroup-radio-button')}></span>
                                        <span className={cx('inputRadioGroup-radio-label')}>
                                ...
                                </div>
                                {
                                    ...
                                    <>
                                        {perkState.listShippingFee?.length > 0 &&
                                            <div className={cx('entreField')}>
                                                <div className={cx('inputDoubleField-headers')} style={{ display: 'flex' }}>
                                                    <div style={{ padding: '6px' }} class='col-8'><label className={cx('entreField-label')} style={{ marginBottom: '0px' }}>Địa điểm vận chuyển</label></div>
                                                    <div style={{ padding: '6px' }} class='col-4'><label className={cx('entreField-label')} style={{ marginBottom: '0px' }}>Phí vận chuyển</label></div>
                                                </div>
                                                {perkState.listShippingFee.map((item, index) => {
                                                    return (
                                                        <ItemShipping key={index} index={index} onChangeItem={handleChangeItemShipping} removeItem={handleRemoveItemShipping} listLocationShip={listLocationShip} lengthListItemShiping={perkState.listShippingFee.length} itemData={item} listLocationShipChoosen={listLocationShipChoosen} />
                                                    )
                                        ...
                                        {
                                            showBtnAddShip &&
                                            <div onClick={handleClickAddShip} style={{ padding: '16px 0', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
