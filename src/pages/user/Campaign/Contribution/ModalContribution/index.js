...
function ModalContribution({ setShowModal, contribution, handleChangeStatus }) {
    ...
    const handleClickSave = async () => {
        if (status !== (contribution.isFinish ? 'Đã gửi' : 'Chưa gửi')) {
            ...
            try {
                const res = await customAxios.patch(`${baseURL}/contribution/editStatus/${contribution._id}`, { isFinish: true })
                if (res) {
    ...
    return (
        <div className={cx('wrapper')}>
            <div className={cx('body')}>
                ...
                <p className={cx('description')}>Tên người dùng hệ thống: {contribution.email ? contribution.user[0].fullName : 'Khách vãng lai'}</p>
                            ...
                            <div style={{ display: 'flex', gap: '48px', marginTop: '8px' }}>
                                <div style={{ width: '40%' }}>
                                    <div className={cx('form-group')}>
                                        ...
                                        <div className={cx('info-value')}>{convertDate(contribution.date)}</div>
                                    </div>
                                    <div className={cx('form-group')}>
                                        <label>Tiền đặc quyền: </label>
                                        <div className={cx('info-value')}>{formatMoney(contribution.perks.reduce((acc, cur) => {
                                    ...
                                    <div className={cx('order-item-wrapper')}>
                                        {
                                            contribution.perks.map((item, index) => {
                                                return (
                                                    <ItemPayment key={index} index={index} item={item} />
                                                )
                                            })
                                        }
                                    </div>
                ...
                <div className={cx('section-button')}>
                    ...
                    <a onClick={() => setShowModal(false)} className={cx('btn', 'btn-cancel')}>Đóng</a>
