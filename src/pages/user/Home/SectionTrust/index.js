    ...
    useEffect(() => {
        const reveal = () => {
            if (element.current) {
                ...
                let opacity2 =
                    1 - (revealTop < 0 ? Math.abs(revealTop) : revealTop - 20) / (windowHeight - revealPoint) + 0.4;
                let opacity3 =
                    1 - (revealTop < 0 ? Math.abs(revealTop) : revealTop + 100) / (windowHeight - revealPoint) + 0.4;
                let opacity4 =
                    1 - (revealTop < 0 ? Math.abs(revealTop) : revealTop + 180) / (windowHeight - revealPoint) + 0.4;
                let opacity5 =
                    1 - (revealTop < 0 ? Math.abs(revealTop) : revealTop + 260) / (windowHeight - revealPoint) + 0.7;
    ...
    return (
        <div className={cx('wrapper', { active: active })} ref={element}>
            <div className={cx('content')}>
                <img style={{ width: '140px', height: '120px' }} src={logoTrangNho} alt="img" />
                ...
                <p className={cx('body')}>
                    ...
                    >
                        Với đội ngũ toàn cầu tận tâm vì sự tin cậy và an toàn, chúng tôi đã quản lý thành công các hoạt
                        động gây quỹ trên toàn thế giới trong hơn một thập kỷ. Đừng lo lắng về bất cứ điều gì, chúng tôi
                        sẽ giúp bạn có những chiến dịch gây quỹ an toàn, thành công và đáp ứng được mọi nhu cầu của bạn
